class AxisWidgetChat {
    ...
    async fetchDownloadFile(attachment) {
        const buttonFile = document.querySelector(`#${attachment.messageId} button`)
    ...
    ChatMessage(message,showUserName = true,showDate = true) {
        const fromUser = message.userId === this.userData.id;
        return `
            <div class="axis-widget-chat-message-container" data-from-user="${fromUser}">
                <div id="${message.id}" class="axis-widget-chat-message" data-from-user="${fromUser}">
                ${showUserName ? `<strong>${message.username}</strong>`:''}

                ${message.attachment ? 
                    `
                        <button class="axis-widget-chat-message-button" onclick='_axisWidgetChat.fetchDownloadFile(${JSON.stringify({...message.attachment, messageId: message.id})})'>
                            ${this.Icon.File}
                ...
                return `
                    <div class="date-group">
                        ...
                        ${group.messages.map((msg,i) => {
                            const showUserName =  msg.userId !== group.messages[i - 1]?.userId && msg.userId !== this.userData.id
                            const showDate = msg.userId !==  group.messages[i + 1]?.userId || i === group.messages.length - 1
                            return this.ChatMessage(msg,showUserName,showDate)}).join('')
                        }
...
class AxisWidgetClass {
    constructor({ userName, userEmail, userId, origin, company, data, isOpen }) {
        ...
        if (!userName || !userEmail || !userId || !origin || !company) throw new Error('Missing required parameters: userName, userEmail, userId, origin, company')
    ...
    fetchAuth() {
        fetch(`${this.baseUrl}/users/guest/verify`, {
            ...
            .then(response => response.json())
            .then(({ disclaimer, tags }) => {
                let state = {};
                if(disclaimer) state.disclaimer = disclaimer;
    ...
    refreshTicketList() {
        ...
        if(this.state.tickets.length > 0) return ticketList.innerHTML =  `
            <strong class="axis-widget-ticket-list-title">Ãšltimos tickets</strong>
            ${this.state.tickets.map(({ title, tag, id, opened }) => this.TicketCard({ id, title, description: `${id} - ${tag}`, opened, icon: this.Icon.ChevronRight })).join('')}
        `
        else return ticketList.innerHTML = `
    ...
    TicketList() {
        function renderContent() {
            if (this.state.isLoading) return '<small class="axis-widget-ticket-list-message">Carregando...</small>'
    ...
    ChatHeader(title = 'Ticket') {
        ...
        ${this.state.currentTab === 'chat' ? `<span class="axis-widget-chat-header-status" data-type="${this.state.selectedTicketOpened ? 'opened' : 'closed'}">
    ...
    updateNewTicketButton() {
        const input = document.querySelector('.axis-widget-new-ticket-form input');
        const select = document.querySelector('.axis-widget-new-ticket-form select');
    ...
    NewTicketForm() {
        return `<form class="axis-widget-new-ticket-form">
            <label>
                <span>TÃ­tulo</span>
                <input 
                    type="text" 
            ...
            <label>
                ...
                <select name="tag" 
                    ...
                    <option value='null'>Selecione uma opÃ§Ã£o...</option>
                    ${this.state.tags.map(tag => `
                        <option value='${tag}' ${this.formFields.tags === tag ? 'selected' : ''}>${tag}</option>
                    `).join('')}
                </select>
            </label>
    ...
    render() {
        ...
        if (currentTab === 'home') {
            ...
            ${this.WelcomeHeader()}
            ${this.TicketCard({ title: 'Iniciar uma nova conversa', description: this.state.disclaimer, id: '', opened: true, icon: this.Icon.Send, currentTab: 'new' })}
            ${this.TicketList()}`;
...
function AxisWidget({ userName, userEmail, userId, origin, company, data = {}, isOpen = false }) {
    window._axisWidget = new AxisWidgetClass({ userName, userEmail, userId, origin, company, data, isOpen })
}
